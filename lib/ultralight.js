      ...
      'lock': ['node', function negotiateLock(callback, results) {
        ...
        async.until(function() {
          ...
          self._getChildren(lockpath, false, function(err, children) {
            ...
            if (err) {
              log.warning('Error obtaining children.', {error: err, path: lockpath, name: lockname});
            ...
            if (nextLowest) {
              ...
              self._exists(nextLowest, true, function(err, exists) {
                ...
                self._addWatchCallback(nextLowest, callback);
                return;
              });
            }
          });
...
/**
 * Keep track of the watch callbacks to be called when the watch is deleted.
 * @param {String} path The Path.
 * @param {Function} watchCallback The function to eventually callback
 */
ZkCxn.prototype._addWatchCallback = function(path, watchCallback) {
  this._watching[path] = this._watching[path] || [];
  this._watching[path].push(watchCallback);
};
