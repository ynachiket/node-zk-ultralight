...
/** Default connection timeout in ms, passed to node-zookeeper. */
var DEFAULT_CXN_TIMEOUT = 8000;
...
exports.getCxn = function getCxn(zkUrls, cxnTimeout) {
  var
    urls = zkUrls.join(','),
    timeout = cxnTimeout || DEFAULT_CXN_TIMEOUT,
...
function ZkCxn(urls, timeout) {
  ...
  this._watching = {}; // node path -> function
  this._timeout = timeout;
  this._sessionId = undefined;
}


// TODO: Rather than emitting events on the ZkCxn instance,
//       emit them on a member EventEmitter, so old listeners
//       can be garbage-collected on reconnect.
/* emits state changes, used for i.e. on('CONNECTED', {Function}) */
util.inherits(ZkCxn, events.EventEmitter);
...
ZkCxn.prototype.onConnection = function(callback) {
  ...
  callback = _.once(function() {
    ...
    // don't use apply(), don't want to change the value of `this`
    originalCallback(arguments[0], arguments[1], arguments[2]);
  });

  this.on(this.cxnStates.CONNECTED, callback);
  this.on(this.cxnStates.ERROR, callback);
};
